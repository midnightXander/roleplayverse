...
BASE_DIR = Path(__file__).resolve().parent #base url, adding .parent brings to the previous folder(parent folder)
...
def sendResetPasswordLink(recipient_email:str,language:str='en'):
    email = EmailMessage()
    email["Subject"] = "Reset password"
    email["From"] = gmail_adress
    email.add_alternative(f"""\
    <html>
    <head>Hello {recipient_email}
    <style>
...
def _parse_number(number:int, show_zero=False ):
    ...
    if number > 1000000:
        parsed = round(number/1000001,1)
        return f"{parsed}M"
    if number == 1000000:
        return f"1M"
    if number > 1000:
        parsed = round(number/1001,2)
        return f"{parsed}K"
    if number == 1000:
        return f"1k"
...
def decrypt_message(encrypted_str:str):
    #decrypt message
    encrypted_message = encrypted_str.encode('utf-8') 
    decrypted_message = fernet.decrypt(encrypted_message)
    return decrypted_message.decode()

import random

def generate_referall_code(username:str):
